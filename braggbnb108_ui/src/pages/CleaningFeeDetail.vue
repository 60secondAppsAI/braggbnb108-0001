					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateCleaningFee()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2CleaningFees = true">Add</base-button>
								              <modal :show.sync="modal2CleaningFees">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Cleaning Fee Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="CleaningFeeId" type="text" placeholder="Enter CleaningFeeId" v-model="cleaningFeeDetails.cleaningFeeId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Amount</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Amount" type="text" placeholder="Enter Amount" v-model="cleaningFeeDetails.amount"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      cleaningFeeToAdd: {},
      modal2CleaningFees: false,
      cleaningFeeDetails: null,
  ...
  methods: {
    ...
    async updateCleaningFee() {
      ...
      const jsonData = JSON.stringify(this.cleaningFeeDetails);
      const res = await CleaningFeeService.update(jsonData);
...
//        this.getAllCleaningFees();
    ...
    async getCleaningFeeDetails() {
      try {
			let response = await CleaningFeeService.get(this.cleaningFeeId);
			this.cleaningFeeDetails = response.data;
